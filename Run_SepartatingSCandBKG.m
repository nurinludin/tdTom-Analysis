...
ActMap(find(peakAmpMap>2*scthresh))=1;
...
CoorMap(~ActMask)=0;
totalArea = length(nonzeros(CoorMap));
% CoorMap(find(CoorMap<0.2))=0;%%%%%%%threshold for activity
totalActiveArea = length(nonzeros(CoorMap));
percentActiveArea = totalActiveArea/totalArea*100;
